...
class ValidationEngine:
    ...
    def _load_findings_from_files(self) -> List[Dict[str, Any]]:
        ...
        Deduplicates by logical vulnerability identity: (url, parameter, type).
        """
            ...
            for json_file in sorted(subdir_path.glob("*.json")):
                try:
                    ...
                    for finding in findings:
                        dedup_key = self._logical_finding_key(finding)
                        existing = results_by_key.get(dedup_key)
                        if existing is not None:
                            self._fill_missing_finding_fields(existing, finding)
                            continue
                        results_by_key[dedup_key] = finding
    ...
    @staticmethod
    def _logical_finding_key(finding: Dict[str, Any]) -> tuple:
        """Return the scan-local identity for one vulnerability."""
        return (
            str(finding.get("url", "")).strip(),
            str(finding.get("parameter", "")).strip(),
            str(finding.get("type", "")).strip().upper(),
        )

    @staticmethod
    def _fill_missing_finding_fields(primary: Dict[str, Any], fallback: Dict[str, Any]) -> None:
        """Keep the higher-priority finding while preserving useful missing metadata."""
    ...
    def _triage_findings(self, findings: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        for f in findings:
            ...
            # Specialists emit NEEDS_CDP_VALIDATION (XSS/CSTI L6); older code only
            # matched PENDING_CDP_VALIDATION, so the flagged findings fell through to
            # nothing. Route both to CDP.
            if status in ("PENDING_CDP_VALIDATION", "NEEDS_CDP_VALIDATION"):
                needs_cdp.append(f)
                continue
    ...
    def _apply_single_cdp_result(self, finding: Dict, result: Dict):
        ...
        if result.get("validated"):
            ...
            # An INCONCLUSIVE CDP/vision re-check must not DOWNGRADE a finding the specialist
            # already CONFIRMED. Browser validation can't observe HTTP-level vulns (SQLi, IDOR,
            # LFI...), so "needs manual review" there means "the browser couldn't check it", not
            # "it's doubtful" — the specialist's evidence (SQL error, quote-parity, data
            # extraction) stands. (A POSITIVE false-positive verdict below is still honoured.)
            if finding.get("status") == "VALIDATED_CONFIRMED":
                dashboard.log(f"↔ Keeping specialist CONFIRMED (CDP re-check inconclusive): {finding.get('type')} on {finding.get('parameter')}", "INFO")
            else:
