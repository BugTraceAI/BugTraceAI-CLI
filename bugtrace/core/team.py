...
<body>
    ...
    async def _phase_3_strategy(self, dashboard, analysis_json_dir: Path) -> int:
        ...
        if detected_csti_framework:
            ...
            spa_urls = sorted(getattr(self, 'urls_to_scan', []))
        ...
        try:
            xml_candidates = await self._harvest_xml_endpoint_candidates()
        except Exception as e:
        ...
        for cand in xml_candidates[:MAX_XXE_PROBES]:
            ...
            if base in existing_xxe_urls:
                continue
            try:
                is_xml = await self._is_xml_endpoint(cand)
            except Exception:
                is_xml = False
            if not is_xml:
                continue
        ...
        if xxe_injected > 0:
            self._v.emit("strategy.auto_dispatch", {"specialist": "XXE", "count": xxe_injected})
        ...
        admin_bac_findings = []
        admin_patterns = ["/admin", "/internal", "/management"]
        admin_urls_to_check = [
        ...
        if admin_urls_to_check:
            import aiohttp as _aiohttp
            # Per-origin catch-all "shell" signature. A SPA (React/Vue/Angular)
            # serves the SAME index.html (HTTP 200) for ANY route, so a 200 on
            # /admin is meaningless unless it differs from a guaranteed-nonexistent
            # sibling path. We probe one junk path per origin and treat any HTML
            # admin response that matches it as a framework shell (false positive),
            # NOT real broken access control. Generic — works on any SPA target.
                            ...
                            async with bac_session.get(admin_url) as resp:
                                if resp.status == 200:
                                    ...
                                    if len(body) > 50:  # Not just an empty 200
                                        # SPA-shell guard: skip HTML responses that match the
                                        # origin's catch-all shell (200 + ~same size as a known-
                                        # nonexistent path). Real BAC = JSON admin data, or HTML
                                        # that differs from the catch-all shell.
                                        if not is_json:
                                            parsed_au = urlparse(admin_url)
                                            origin = f"{parsed_au.scheme}://{parsed_au.netloc}"
                                            j_status, j_len = await _catch_all_signature(bac_session, origin)
                                            if j_status == 200 and abs(j_len - len(body)) <= 64:
                                                logger.debug(
                                                    f"[BAC] SPA-shell FP skipped: {admin_url} "
                                                    f"(matches catch-all shell, {len(body)}b ~ {j_len}b)"
                                                )
                                                continue
