...
async def dispatch_specialists(
    ...
    # Log any specialist that crashed (without aborting the rest)
    for specialist_name, result in zip(activated, results):
        if isinstance(result, Exception):
            logger.error(f"[Dispatcher] {specialist_name} crashed: {result}")
