...
class LLMClient:
    ...
    # NOTE: no @retry decorator here. `_try_generate_with_models` already retries
    # each model (with typed-exception backoff) and shifts across models, so an
    # outer tenacity retry re-ran the ENTIRE model-shifting sequence — multiplying
    # API calls and bumping consecutive_errors fast enough to trip the circuit
    # breaker (which then fails open to CONFIRMED). The inner loop is sufficient.
    async def generate(
        self,
        prompt: str,
        module_name: str,
