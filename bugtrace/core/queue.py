...
class SpecialistQueue:
            ...
            if self.file_path != file_path:
                ...
                while not self._queue.empty():
                    try:
                        ...
                        break
                # Reset counters for the new scan: depth() = file_lines - total_dequeued must be
                # per-scan-correct on this reused singleton queue. Without this, the previous
                # scan's dequeues would subtract from the new scan's file and under-report pending
                # items (risking a skipped Phase-A drain). The new tailer re-counts from zero.
                self._stats.reset()
    ...
    def depth(self) -> int:
        """Get current PENDING queue depth — items written but not yet consumed."""
        # In file mode the WET file is APPEND-ONLY: its line count is every item ever produced and
        # never shrinks on consume, so counting lines alone over-reports — a fully-drained scan
        # still shows every historical item (the source of the bogus "N items remaining" warning).
        # Subtract what has already been dequeued to get the TRUE pending count. We still count
        # FILE lines (not just the in-memory buffer) so the reading covers the race where the
        # producer has written lines the tailer hasn't read yet. total_dequeued is reset per scan
        # in enable_file_mode(), so this stays per-scan-correct on reused (singleton) queues.
        if self._file_mode and self.file_path and self.file_path.exists():
            try:
                with open(self.file_path, 'r') as f:
                    total_lines = sum(1 for line in f if line.strip())
                return max(0, total_lines - self._stats.total_dequeued)
            except Exception as e:
