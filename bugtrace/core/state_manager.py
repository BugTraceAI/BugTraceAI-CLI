    ...
    def save_state(self, state_data: Dict[str, Any]):
        ...
        if self.scan_id:
            try:
                ...
                # NOTE: do NOT return here. load_state() reads ONLY the target-keyed
                # file (the DB is write-only from the CLI), so resume restores nothing
                # unless we ALSO persist the file checkpoint below.
            except Exception as e:
                logger.error(f"Failed to save state to DB: {e}, falling back to file.", exc_info=True)

        # Always write the file checkpoint — this is what load_state()/resume reads.
        try:
            wrapper = {
                "target": self.target,
