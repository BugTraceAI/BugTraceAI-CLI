...
# Definitive validator verdicts that are authoritative on update (may downgrade).
...
def _resolve_status(current: FindingStatus, incoming, validated: bool) -> FindingStatus:
    ...
    Rules (in order):
      1. A positive validation signal (validated/conductor_validated) always
         confirms — VALIDATED_CONFIRMED wins.
      2. Otherwise an EXPLICIT terminal verdict from the validator
         (confirmed / false-positive / manual-review) is honored, so a finding
         the auditor rejected can be DOWNGRADED (the previous code only ever moved
         status up, silently discarding rejections — what the API/WEB then read).
      3. Partial/unknown signals (pending, error, missing, unparseable) leave the
         current status untouched, so a later weak write never clobbers a strong
         one.
    """
    if validated:
        return FindingStatus.VALIDATED_CONFIRMED
    if incoming:
        try:
            candidate = incoming if isinstance(incoming, FindingStatus) else FindingStatus(incoming)
        except ValueError:
            return current
        if candidate in _TERMINAL_VERDICTS:
            return candidate
    return current
