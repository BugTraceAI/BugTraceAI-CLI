...
class ConductorV2:
    ...
    @staticmethod
    def _fresh_shared_context() -> Dict[str, Any]:
        return {
            "discovered_urls": [],
            "confirmed_vulns": [],
            "tested_params": [],
...
"""
    ...
    def verify_integrity(self, phase: str, expected: Dict, actual: Dict) -> bool:
        ...
        if phase == "discovery":
            ...
            logger.error(f"[Conductor] Unknown phase for integrity check: {phase}")
            self.stats["integrity_failures"] = self.stats.get("integrity_failures", 0) + 1
            return False
    ...
    def begin_scan(self, scan_id: int) -> None:
        """Start an isolated scan context for this global conductor."""
        with self._context_lock:
            self._scan_context = str(scan_id)
            self.shared_context = self._fresh_shared_context()
