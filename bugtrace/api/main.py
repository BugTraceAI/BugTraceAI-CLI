...
async def _ws_stream_live_events(websocket: WebSocket, scan_id: int, highest_sent: int):
    """Stream live events with deduplication and disconnect detection."""
    stream_task = asyncio.create_task(
        _ws_drain_event_stream(websocket, scan_id, highest_sent)
    )
        ...
        while not stream_task.done():
            ...
            if receive_task in done:
                try:
                    ...
                    logger.info(f"WebSocket receive ended for scan {scan_id}")
                    break
                continue
