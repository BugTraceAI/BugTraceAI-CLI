...
_TERMINAL_SCAN_STATUSES = {"COMPLETED", "STOPPED", "FAILED", "CANCELLED"}
_METRICS_UNAVAILABLE_REASON = "Detailed metrics are real-time only and are not persisted."
...
async def get_detailed_metrics(
    ...
    try:
        ...
        status_dict = await scan_service.get_scan_status(scan_id)
        scan_status = str(status_dict["status"]).upper()
        metrics_available = scan_id in scan_service.get_active_scan_ids() and scan_status not in _TERMINAL_SCAN_STATUSES
        ...
        return {
            ...
            "metrics_unavailable_reason": None if metrics_available else _METRICS_UNAVAILABLE_REASON,
