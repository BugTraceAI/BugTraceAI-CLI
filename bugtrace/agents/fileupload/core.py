...
def create_upload_finding(
    ...
    """
    # Execution is proven ONLY by a real execution check or our RCE flag appearing in the
    # response. A server merely echoing the uploaded FILENAME back is an accepted upload,
    # NOT proof of code execution — so it must not be stamped CRITICAL/confirmed RCE.
    executed = valid_execution or ("RCE_FLAG" in response_text)

    if "RCE_FLAG" in response_text:
        evidence = f"RCE Flag Found in response: {response_text}"
    elif executed:
        evidence = f"Uploaded file executed on the server. Response: {response_text[:100]}"
    else:
        evidence = f"Upload accepted and filename reflected (execution NOT confirmed): {response_text[:100]}"

    finding = {
        "type": "File Upload / RCE" if executed else "File Upload",
        "vulnerability": "Unrestricted File Upload / RCE" if executed else "Unrestricted File Upload (execution unconfirmed)",
        "url": url,
        "exploit_url": uploaded_url,
        "filename": filename,
        "confidence": 1.0 if executed else 0.6,
        "method": "RCE via Execution" if executed else "Arbitrary File Upload (Name Trigger — execution unconfirmed)",
        "evidence": evidence,
        "validated": True,
        "severity": "CRITICAL" if executed else "HIGH",
        "status": "VALIDATED_CONFIRMED",
        "description": (
            f"Unrestricted file upload allowing Remote Code Execution. Uploaded file "
            f"'{filename}' was executed on the server."
            if executed else
            f"File upload restriction bypass: '{filename}' was accepted and its name "
            f"reflected in the response, but code execution was NOT confirmed — verify "
            f"whether the file is served/executed at the exploit URL."
        ),
        "reproduction": (
            f"# Upload file:\n"
            f"curl -X POST '{url}' -F 'file=@{filename}'\n"
            f"# Check if served/executed:\n"
            f"curl '{uploaded_url}'"
        ),
    }
    if not executed:
