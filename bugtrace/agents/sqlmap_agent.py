...
class EnhancedSQLMapRunner:
    ...
    def _add_cookies(self, cmd: List[str]):
        ...
        try:
            ...
            # Tell SQLMap to test cookies as injection points. MERGE them into the
            # existing -p list instead of appending a second -p: sqlmap keeps only the
            # LAST -p, so a second one would silently drop the URL/POST param under test.
            if cookie_names:
                if "-p" in cmd:
                    idx = cmd.index("-p")
                    existing = cmd[idx + 1]
                    cmd[idx + 1] = ",".join([p for p in [existing, *cookie_names] if p])
                else:
                    cmd.extend(["-p", ",".join(cookie_names)])
                logger.info(f"Testing cookie parameters (merged with -p): {cookie_names}")
...
class SQLMapAgent(BaseAgent):
    ...
    async def _test_error_payloads(self, session, base_url: str, existing_params: Dict,
        ...
        # Baseline: the same request with a benign param value. A SQL error only counts as
        # injection when it is ABSENT from this baseline — otherwise a page that already
        # shows a DB error (docs, a WAF page echoing the payload) would false-positive.
        baseline_body = await self._fetch_error_baseline(
            session, base_url, existing_params, param, req_headers
        )
        for payload in payloads:
            finding = await self._test_payload_for_error(
                session, base_url, existing_params, param, payload, req_headers, baseline_body
            )
            if finding:
                return finding
        return None

    async def _fetch_error_baseline(self, session, base_url: str, existing_params: Dict,
                                    param: str, req_headers: Dict) -> str:
        """Fetch the response body for a benign param value (baseline for the error differential)."""
        test_params = {k: v[0] if isinstance(v, list) else v for k, v in existing_params.items()}
        if not test_params.get(param):
            test_params[param] = "1"
        url = f"{base_url}?{urlencode(test_params)}"
        try:
            async with session.get(url, headers=req_headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
    ...
    async def _test_payload_for_error(
        ...
        param: str, payload: str, req_headers: Dict, baseline_body: str = ""
    ) -> Optional[Dict]:
        ...
        async with session.get(test_url, headers=req_headers, timeout=aiohttp.ClientTimeout(total=10)) as response:
            ...
            for pattern in patterns:
                ...
                # Only confirm when the error appears under the payload but NOT in the benign
                # baseline — i.e. the injection caused it, not a page that always shows it.
