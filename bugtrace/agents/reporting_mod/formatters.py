...
def _build_xxe_steps(finding: Dict) -> List[str]:
    ...
    # Derive the injection point from the finding itself — the XXE endpoint is the finding's
    # own URL, never a hardcoded path (was pinned to ginandjuice's /catalog/product/stock,
    # which 404s on any other target).
    url = finding.get("url", "")
