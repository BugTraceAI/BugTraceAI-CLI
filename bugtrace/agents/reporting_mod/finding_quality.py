...
from bugtrace.agents.reporting_mod.finding_baseline import _safe_evidence_get
...
# Endpoint-scoped families: collapsing only by param loses distinct URLs
# (e.g. Mass Assignment auto_dispatch × 19 endpoints → 1).
_ENDPOINT_SCOPED_TYPES = frozenset({
    "MASS ASSIGNMENT",
...
_GENERIC_DISPATCH_PARAMS = frozenset({
    "auto_dispatch",
    "_auto_discover",
    "n/a",
    "na",
...
# PURE
def finding_dedup_key(finding: Dict) -> tuple:
    """Identity for presentation dedup (type, param[, path when endpoint-scoped])."""
    param_raw = finding.get("parameter", "") or ""
    param_normalized = normalize_parameter_for_dedup(param_raw)
    vuln_type = normalize_type_for_dedup(finding.get("type", "Unknown"))
    path = ""
    if (
        param_normalized in _GENERIC_DISPATCH_PARAMS
        or vuln_type in _ENDPOINT_SCOPED_TYPES
        or finding.get("_auto_dispatched")
    ):
        try:
            path = (urlparse(finding.get("url") or "").path or "").rstrip("/") or "/"
        except Exception:
            path = (finding.get("url") or "").strip() or "/"
    return (vuln_type, param_normalized, path)


# PURE
def deduplicate_findings(findings: List[Dict]) -> List[Dict]:
    """
    Deduplicate findings by (type, normalized_parameter[, path]).

    Same param on many URLs collapses for injection-style bugs (SQLi cookie).
    Endpoint-scoped / auto_dispatch keep one finding per path so Mass Assignment
    and BAC do not collapse 19 endpoints into one row.

    Returns: List of deduplicated findings with 'affected_urls' metadata.
    """
    groups = defaultdict(list)
    for f in findings:
        groups[finding_dedup_key(f)].append(f)

    deduplicated = []
    for key, group in groups.items():
        vuln_type, param_key = key[0], key[1]
        if len(group) == 1:
...
def _nuclei_template_id(finding: Dict) -> str:
    """Extract nuclei template id from evidence/parameter (PURE)."""
    return str(
        _safe_evidence_get(finding, "nuclei_template", finding.get("parameter", "")) or ""
    ).lower()


def _is_security_header_template(tmpl: str) -> bool:
    """True if template belongs in the consolidated missing-headers group (PURE)."""
    if not tmpl:
        return False
    if tmpl in HEADER_TEMPLATES:
        return True
    # Prefix match: dual/orig routing + nuclei variants beyond the static set
    return tmpl.startswith("security-headers-") or tmpl.startswith("http-missing-security")
...
def consolidate_informational(findings: List[Dict]) -> List[Dict]:
    ...
    for f in findings:
        tmpl = _nuclei_template_id(f)
        ftype = f.get("type", "").upper()

        if ftype == "MISSING_SECURITY_HEADER" and _is_security_header_template(tmpl):
...
# PURE
def present_findings_for_api(findings: List[Dict]) -> List[Dict]:
    """Canonical presentation pipeline for GET /findings (PURE composition).

    Type canonicalize/infer + drop Unknown dry stubs → exact dedup → type/param
    collapse → informational consolidation. Does not mutate inputs.
    """
    if not findings:
        return []
    from bugtrace.core.finding_type_policy import prepare_findings_for_presentation

    # Type policy returns shallow copies; still re-copy for safety downstream
    working = prepare_findings_for_presentation(list(findings))
    working = [dict(f) for f in working]
    working = deduplicate_exact(working)
    working = deduplicate_findings(working)
    working = consolidate_informational(working)
    return working
