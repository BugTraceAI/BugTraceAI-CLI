"""
RCE / Command Injection specialist agent -- thin orchestrator.
...
from bugtrace.agents.rce.detection import (
    validate_before_emit,
    get_time_payloads,
    create_time_based_finding,
    create_eval_finding,
    create_output_finding,
    create_deserialization_finding,
    inject_payload,
    generate_rce_fingerprint,
    fallback_fingerprint_dedup,
    check_deser_keywords,
    exploit_dry_sort_key,
    DESER_KEYWORDS,
)
...
class RCEAgent(BaseAgent, TechContextMixin):
    ...
    async def _test_parameter(self, session, param: str, payloads: List[str]) -> Optional[Dict]:  # I/O
        ...
        for payload_idx, p in enumerate(payloads, 1):
            if hasattr(self, '_v'):
                self._v.progress("exploit.specialist.progress", {
                    "agent": "RCE", "param": param, "payload": p[:60],
                }, every=50)
    ...
    async def _test_single_payload(self, session, param: str, payload: str) -> Optional[Dict]:  # I/O
    ...
    async def _test_time_based(self, session, param: str, payload: str) -> Optional[Dict]:  # I/O
        ...
        if elapsed >= 5:
            return create_time_based_finding(self.url, param, payload, elapsed)
        return None

    async def _test_eval_based(self, session, param: str, payload: str) -> Optional[Dict]:  # I/O
        """Test eval-based RCE payload."""
        dashboard.update_task(f"RCE:{param}", status=f"Testing Eval: {payload}")
        target = inject_payload(self.url, param, payload)

        try:
            async with session.get(target) as resp:
                text = await resp.text()
                if "Result: 2" in text:
                    return create_eval_finding(self.url, param, payload)
    ...
    async def _test_output_based(self, session, param: str) -> Optional[Dict]:  # I/O
        ...
        for cmd, pattern in output_payloads:
            target = inject_payload(self.url, param, cmd)
            try:
                async with session.get(target, timeout=10) as resp:
                    text = await resp.text()
                    if resp.status == 200:
                        if cmd == "echo BTAIRCE7331" and "BTAIRCE7331" in text:
                            return create_output_finding(self.url, param, cmd, "BTAIRCE7331")
                        elif cmd == "id" and _re.search(r"uid=\d+", text):
                            return create_output_finding(
                                self.url, param, cmd,
                                _re.search(r"uid=\d+", text).group(),
                            )
                        elif cmd == "whoami":
                            match = _re.search(r"\b[a-z_][a-z0-9_-]{0,31}\b", text)
                            if match and match.group() not in {param, cmd}:
                                return create_output_finding(self.url, param, cmd, match.group())
            except Exception:
                continue
        return None

    async def _test_deserialization(self, url: str, param: str) -> Optional[Dict]:  # I/O
    ...
    async def _discover_rce_params(self, url: str) -> Dict[str, str]:  # I/O
        """
        ...
        from bugtrace.agents.rce.detection import RCE_PRIORITY_KEYWORDS
        ...
        # 3. Prioritize RCE-related parameters
        prioritized_params = {}
        ...
        high_priority = [
            p for p in prioritized_params.keys()
            if any(k in p.lower() for k in RCE_PRIORITY_KEYWORDS)
        ]
        if high_priority:
            logger.info(f"[{self.name}] Found {len(high_priority)} HIGH-PRIORITY RCE params: {high_priority}")
...
Treat each parameter as a SEPARATE potential injection point - do not merge them."""
    ...
    async def exploit_dry_list(self) -> List[Dict]:  # I/O
        ...
        # Sort: cookies/deserialization first, URL params last
        sorted_findings = sorted(self._dry_findings, key=exploit_dry_sort_key)
        ...
        for idx, f in enumerate(sorted_findings, 1):
            ...
            try:
                self.url = url
                # Deserialization findings need cookie-based testing
                ...
                if hasattr(self, '_v'):
                    self._v.emit("exploit.specialist.param.completed", {
                        "agent": "RCE", "param": param, "url": url, "idx": idx,
                    })
            ...
            try:
                ...
                if auth_headers:
                    ...
                    async with aiohttp.ClientSession(headers=auth_headers) as auth_session:
                        ...
                        if result:
                            return result
                        # Also try output-based detection
    ...
    async def _handle_queue_result(self, item: dict, result: Optional[Dict]) -> None:  # I/O
        ...
        fingerprint = generate_rce_fingerprint(result.get("url", ""), result.get("parameter", ""))
    ...
    async def _load_rce_tech_context(self) -> None:  # I/O
