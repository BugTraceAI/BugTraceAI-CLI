...
class OpenRedirectAgent(BaseAgent, TechContextMixin):
    ...
    def _discover_param_vectors(self) -> List[Dict]:
        ...
        # Active probing: on endpoints whose PATH signals a redirector (e.g. /api/redirect,
        # /go, /out, /link), the vulnerable param is usually NOT present in the crawled URL —
        # so the param-discovery above finds nothing. Probe the well-known redirect param
        # names directly instead of waiting for one to appear. Non-matching names just 4xx.
        path_lower = (parsed.path or "").lower()
        if any(sig in path_lower for sig in ("redirect", "redir", "/go", "/out", "/link", "/away", "/exit", "/leave", "/jump")):
            probe_params = ["url", "next", "redirect", "redirect_url", "redirect_uri", "redirectUrl",
                            "return", "returnUrl", "returnTo", "dest", "destination", "target", "goto", "continue", "to", "u"]
            for pp in probe_params:
                if not any(v["param"].lower() == pp.lower() for v in vectors):
                    vectors.append({
                        "type": "QUERY_PARAM",
                        "param": pp,
                        "value": "",
                        "source": "PATH_ACTIVE_PROBE",
                        "confidence": "MEDIUM"
                    })
    ...
    async def _test_content_vector(self, vector: Dict) -> Optional[Dict]:
        ...
        # A static external redirect found on the page is NOT an open redirect — the site
        # simply redirects/links elsewhere. It is only a finding when USER INPUT controls
        # the destination, which the dynamic-marker check below verifies. (Removed the
        # branch that flagged every external redirect host as exploitable — it auto-confirmed
        # legitimate third-party redirects, e.g. a hardcoded <meta refresh> to a CDN.)
...
"""
        ...
        response = await llm_client.generate(
            prompt=prompt,
            system_prompt=self.system_prompt + "\n\nYou are an expert security analyst specializing in Open Redirect deduplication with autonomous parameter discovery.",
    ...
    _REDIRECT_PARAM_KEYWORDS = (
        "redirect", "redir", "next", "return", "returnurl", "goto", "dest",
        "destination", "continue", "callback", "url", "rurl", "target", "to",
        "out", "link", "forward",
    )

    def _resolve_redirect_target(self, url: str, parameter: str, finding: Dict) -> Tuple[str, str]:
        """Recover the true redirect endpoint when the candidate URL was mis-attributed.

        DAST/LLM analysis sometimes tags an open redirect against the page it was
        reading (e.g. a debug/vuln-listing endpoint) while the real target only
        survives in the finding's ``payload``/``exploitation_strategy`` as a concrete
        ``/path?param=value``. Validating the attributed URL then fails because that
        endpoint does not redirect. Parse those fields and, when they point at a
        *different* endpoint, return the corrected ``(url, parameter)`` so Phase B
        tests the endpoint that actually redirects.

        Conservative: only overrides when a concrete path is found that differs from
        the attributed one; otherwise returns the inputs unchanged.
        """
        if not isinstance(finding, dict):
            return url, parameter

        p = urlparse(url) if url else None
        base = f"{p.scheme}://{p.netloc}" if (p and p.scheme and p.netloc) else ""
        target_host = p.netloc if p else ""
        orig_path = p.path if p else ""

        for raw in (finding.get("payload"), finding.get("exploitation_strategy")):
            if not raw or not isinstance(raw, str):
                continue
            # Only consider tokens shaped like an endpoint that CARRIES a query string
            # ("endpoint?param=value"). A bare destination URL (e.g. the attacker host
            # in "url=https://evil.example/...") has no query of its own and must NOT be
            # mistaken for the redirect endpoint.
            for m in re.finditer(r"(https?://[^\s'\"]+?|/[^\s'\"]*?)\?([^\s'\"]+)", raw):
                endpoint_raw, query = m.group(1), m.group(2)
                ep = urlparse(endpoint_raw)

                if ep.scheme and ep.netloc:
                    # Absolute endpoint: must be on the SAME host as the target — an
                    # absolute URL on another host is the redirect *destination*, not
                    # the vulnerable endpoint, and testing it would probe a foreign host.
                    if target_host and ep.netloc != target_host:
                        continue
                    endpoint_path = ep.path or "/"
                    endpoint_url = f"{ep.scheme}://{ep.netloc}{endpoint_path}"
                elif endpoint_raw.startswith("/") and base:
                    endpoint_path = ep.path or "/"
                    endpoint_url = urljoin(base + "/", endpoint_path.lstrip("/"))
                else:
                    continue

                # Only correct when the endpoint actually differs from the attributed one.
                if not endpoint_path or endpoint_path == orig_path:
                    continue

                # The query must name a redirect-style parameter, otherwise this isn't
                # a redirect endpoint and we leave the candidate untouched.
                qs = parse_qs(query)
                new_param = next(
                    (k for k in qs if any(kw in k.lower() for kw in self._REDIRECT_PARAM_KEYWORDS)),
                    "",
                )
                if not new_param:
                    continue
                ...
                logger.info(
                    f"[{self.name}] 🔧 Re-attributed redirect target from payload: "
                    f"{url}?{parameter} → {endpoint_url}?{new_param}"
                )
                return endpoint_url, new_param

        return url, parameter

    # Conventional endpoints that frequently implement an open redirect, probed
    # with a canary when the LLM-supplied candidate is unreliable.
    _COMMON_REDIRECT_PATHS = (
        "/api/redirect", "/redirect", "/api/redir", "/redir", "/go", "/api/go",
        "/out", "/url", "/link", "/away", "/exit", "/goto", "/jump", "/forward", "/r",
    )
    _REDIRECT_CANARY = "https://redirect-canary.bugtrace.example/probe"

    async def _discover_redirect_endpoints(self, origin: str, params: List[str]) -> List[Tuple[str, str]]:
        """Deterministically locate open-redirect endpoints on the target origin.

        LLM-derived candidates frequently mis-attribute the redirect URL (tagging
        the page that *documents* the bug, e.g. a debug/vuln-listing endpoint) or
        omit the endpoint entirely, so the true endpoint never reaches Phase B. As a
        safety net, probe a small set of conventional redirect endpoints with a unique
        canary and keep those whose ``Location`` header reflects it — a deterministic,
        version-independent confirmation. Returns ``[(endpoint_url, param), ...]`` to
        feed the existing validator.
        """
        if not origin:
            return []

        test_params: List[str] = []
        for p in list(params or []) + ["url", "redirect", "next", "dest", "to", "target"]:
            if p and p not in test_params:
        ...
        try:
            async with orchestrator.session(DestinationType.TARGET) as session:
                for path in self._COMMON_REDIRECT_PATHS:
                    if len(found) >= 3 or probes >= 60:
                        break
                    for param in test_params:
                        probes += 1
                        test_url = f"{origin}{path}?{param}={self._REDIRECT_CANARY}"
                        try:
                            async with session.get(
                                ...
                                if resp.status in REDIRECT_STATUS_CODES:
                                    ...
                                    if loc and ("redirect-canary.bugtrace.example" in loc
                                                or self._REDIRECT_CANARY in loc):
                                        found.append((f"{origin}{path}", param))
                                        logger.info(
                                            f"[{self.name}] 🎯 Canary-confirmed open-redirect endpoint: "
                                            f"{origin}{path}?{param}= → Location: {loc[:80]}"
                                        )
                                        break  # endpoint confirmed; move to next path
                        except (aiohttp.ClientError, asyncio.TimeoutError):
                            continue
                        except Exception:
                            continue
        except Exception as e:
            logger.debug(f"[{self.name}] Redirect endpoint discovery skipped: {e}")
        return found
    ...
    async def exploit_dry_list(self) -> List[Dict]:
        ...
        # Deterministic safety net: probe conventional redirect endpoints on the
        # target origin so a real open redirect is still caught when the LLM
        # candidate mis-attributes or omits the endpoint. Confirmed endpoints are
        # appended as candidates and validated by the same Phase B machinery below.
        try:
            origins = set()
            cand_params: List[str] = []
            for fd in self._dry_findings:
                if not isinstance(fd, dict):
                    continue
                pp = urlparse(fd.get("url", ""))
                if pp.scheme and pp.netloc:
                    origins.add(f"{pp.scheme}://{pp.netloc}")
                par = fd.get("parameter", "")
                if par and par not in cand_params:
                    cand_params.append(par)
            existing_targets = {
                (fd.get("url", "").split("?")[0], fd.get("parameter", ""))
                for fd in self._dry_findings if isinstance(fd, dict)
            }
            for origin in origins:
                for eurl, eparam in await self._discover_redirect_endpoints(origin, cand_params):
                    if (eurl, eparam) in existing_targets:
                        continue
                    existing_targets.add((eurl, eparam))
                    self._dry_findings.append({
                        ...
                        "finding": {
                            ...
                            "url": eurl,
                            "severity": "Medium",
                            "_discovered_by": "canary_endpoint_probe",
                        },
                    })
        except Exception as e:
            logger.debug(f"[{self.name}] Redirect endpoint discovery integration skipped: {e}")
        ...
        for idx, finding_data in enumerate(self._dry_findings, 1):
            ...
            # The attributed URL can be mis-pointed at the page the LLM analysed
            # (e.g. a debug/vuln-listing endpoint); recover the real redirect
            # endpoint from the finding payload so validation hits what redirects.
            # Mutate finding_data in place so Phase B.2 (DOM) and emission agree.
            url, parameter = self._resolve_redirect_target(url, parameter, finding)
            finding_data["url"] = url
            finding_data["parameter"] = parameter
