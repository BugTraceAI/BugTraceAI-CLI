...
def analyze_token_response(
    ...
    """  # PURE
    # Clearest bypass: the endpoint rejected the baseline (401/403) but accepted the forged
    # token (200).
    if base_status in [401, 403] and status == 200:
        return True

    # Both returned 200: a generic success/fail WORD delta (admin/success/invalid) is not
    # proof — two benign 200 pages differ on such words all the time. Require a real
    # privilege/state difference (new JSON keys, >=2 new privilege signals, or materially
    # more data) via the canonical comparator instead of hardcoded keyword lists.
    if status == 200:
        return body_shows_privilege_difference(base_text, text)
