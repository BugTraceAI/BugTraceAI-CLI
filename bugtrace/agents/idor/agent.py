...
class IDORAgent(BaseAgent, TechContextMixin):
    ...
    def _create_idor_finding(self, hit: Dict, param: str, original_value: str) -> Dict:
        ...
        diff_type = str(hit.get("diff_type", ""))
        has_authorization_evidence = (
            bool(hit.get("authorization_context"))
            or hit.get("contains_sensitive", False)
            or "permission_bypass" in diff_type
            or "user_data_leakage" in diff_type
        )
        status = (
            determine_validation_status("differential", confidence_level)
            if has_authorization_evidence
            else "PENDING_VALIDATION"
        )
        baseline_url = inject_id(self.url, original_value, param, original_value)
        exploit_url = inject_id(self.url, str(hit["id"]), param, original_value)
        return {
            "type": "IDOR",
            "url": self.url,
            "parameter": param,
            ...
            "evidence": {
                ...
                "authorization_context": bool(hit.get("authorization_context")),
            ...
            "cve_id": "N/A",
            "http_request": f"GET {exploit_url}",
...
{{"ids": ["id1", "id2", "id3", ...]}}"""
        ...
        try:
            ...
            if not response:
                return []
            from bugtrace.utils.json_parser import safe_json_loads
...
{{"is_idor": true/false, "severity": "HIGH"|"MEDIUM"|"LOW", "reasoning": "brief 1-sentence explanation"}}"""
    ...
    async def _exploit_deep(self, finding: Dict) -> Dict:
        ...
        if not phase1.get("confirmed"):
            ...
            # The re-test rebuilds baseline/exploit from the probe's (often synthetic)
            # original_value, so its strict gate (exploit==200, baseline in [200,401,403],
            # bodies differ) can fail to reproduce the differential even for a REAL IDOR that
            # already leaked sensitive data on detection (e.g. /api/orders with user_id=test as
            # the baseline → 422, not in the allowed set). Don't bury such a finding in PENDING:
            # route it to MANUAL_REVIEW (surfaced + reportable, human-verify) rather than
            # auto-confirming — preserving FP discipline. Weak-evidence retests still → PENDING.
            ev = finding.get("evidence", {}) or {}
            strong_evidence = bool(ev.get("user_data_leakage")) or bool(ev.get("contains_sensitive"))
            if strong_evidence:
                finding["status"] = "MANUAL_REVIEW_RECOMMENDED"
                finding["validation_requires_cdp"] = False
            else:
                finding["status"] = ValidationStatus.PENDING_VALIDATION.value
                finding["validation_requires_cdp"] = True
            return finding
