...
class APISecurityAgent(BaseAgent):
    ...
    async def start_queue_consumer(self, scan_context: str) -> None:
        ...
        self._v.emit("exploit.specialist.started", {"agent": "APISecurity", "queue_depth": initial_depth})
        ...
        if not dry_list:
            ...
            self._v.emit("exploit.specialist.completed", {"agent": "APISecurity", "dry_count": 0, "vulns": 0})
            return
        ...
        self._v.emit("exploit.specialist.completed", {"agent": "APISecurity", "dry_count": len(dry_list), "vulns": vulns_count})
    ...
    async def analyze_and_dedup_queue(self) -> List[Dict]:
        ...
        # Deduplicate by URL (one test per unique endpoint)
        seen_urls = set()
        for wf in wet_findings:
            parsed = urlparse(wf["url"])
            # Normalize: scheme + host + path (ignore query params for dedup)
        ...
        for idx, f in enumerate(self._dry_findings, 1):
            ...
            try:
                ...
                if base_key not in graphql_tested:
                    ...
                    graphql_endpoint = await self._discover_graphql_endpoint(url)
                    if graphql_endpoint:
                        result = await self._test_graphql_endpoint(graphql_endpoint)
                ...
                result = await self._test_rest_endpoint(url)
                if result.get("vulnerabilities"):
                    for vuln in result["vulnerabilities"]:
                        vuln["status"] = ValidationStatus.VALIDATED_CONFIRMED.value
                        vuln["validated"] = True
    ...
    async def _discover_graphql_endpoint(self, base_url: str) -> Optional[str]:
        """Discover GraphQL endpoint from a base URL by probing common paths."""
        parsed = urlparse(base_url)
        base = f"{parsed.scheme}://{parsed.netloc}"

        # Check if URL itself is a GraphQL endpoint
        graphql_paths = [
            parsed.path,  # Original path first
            "/graphql",
            "/api/graphql",
            "/graphql/v1",
            "/api/v1/graphql",
                    ...
                    try:
                        ...
                        if resp.status_code == 200:
                            ...
                            if "data" in data or "errors" in data:
                                logger.info(f"[{self.name}] GraphQL endpoint found: {test_url}")
                                return test_url
                    except Exception:
                        continue
        except Exception as e:
            logger.debug(f"[{self.name}] GraphQL discovery error: {e}")
