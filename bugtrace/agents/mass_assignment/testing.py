...
async def test_method_with_fields(
    ...
    for group_name, fields in field_groups.items():
        ...
        try:
            ...
            # Confirmation requires a follow-up GET showing the injected field PERSISTED and
            # CHANGED vs the baseline. The mutation RESPONSE alone just echoes our input
            # (create/echo endpoints return the posted object), which is not proof of binding —
            # so response-echo (check_field_acceptance) is no longer treated as a confirmation.
            accepted_fields = []
            if resp.status_code in (200, 201, 204):
                accepted_fields = await check_followup_get(
