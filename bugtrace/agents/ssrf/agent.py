"""SSRFAgent shell — canonical package owner (P4-SSRF-1/2).

Live team/CLI historically imported ``bugtrace.agents.ssrf_agent``.
That path is now a thin re-export of this module. Pure helpers live in
``bugtrace.agents.ssrf.detection``.
"""
...
# Pure owner: bugtrace.agents.ssrf.detection (P4-SSRF-0). Re-exported for
# smoke_confirm and historical ``from …ssrf_agent import is_time_based_confirmed``.
from bugtrace.agents.ssrf.detection import (  # noqa: E402
    determine_validation_status as _pure_determine_validation_status,
    fallback_fingerprint_dedup as _pure_fallback_fingerprint_dedup,
    generate_ssrf_fingerprint as _pure_generate_ssrf_fingerprint,
    get_validation_status as _pure_get_validation_status,
...
class SSRFAgent(BaseAgent, TechContextMixin):
    ...
    def _emit_ssrf_finding(self, finding_dict: Dict, scan_context: str = None) -> Optional[Dict]:
        ...
        """
        # Build a new enriched dict (no mutation of the caller's). BaseAgent's
        # emit_finding validator REQUIRES a non-empty "url"; SSRF finding dicts
        # only carry "param", so without this every finding was rejected with
        # "Missing target URL" and nothing ever reached the event bus.
        enriched = {
            "type": "SSRF",
            "url": self.url,
            **finding_dict,
        ...
        if not enriched.get("url"):
            enriched["url"] = self.url
        if "parameter" not in enriched and enriched.get("param"):
            enriched["parameter"] = enriched["param"]
        if scan_context:
            enriched["scan_context"] = scan_context
        return self.emit_finding(enriched)
    ...
    async def _test_strategy_payloads(self, strategy: Dict, param: str, findings: List) -> List:
        ...
        baseline = await self._measure_baseline(param)
        ...
        for payload_item in strategy["payloads"]:
            ...
            if res and self._determine_validation_status(res, baseline_elapsed=baseline):
                if hasattr(self, '_v'):
    ...
    async def _measure_baseline(self, param: str) -> Optional[float]:
        """Measure a non-malicious request for this parameter before timing checks."""
        parsed = urlparse(self.url)
        params = parse_qs(parsed.query)
        baseline_payload = params.get(param, [""])[0] or "baseline"
        result = await self._test_payload(param, baseline_payload)
        return result.get("elapsed") if result else None
        ...
        for param in self.params:
            ...
            if findings:
                ...
                for finding in findings:
                    self._emit_ssrf_finding(finding, scan_context=self._scan_context)
    ...
    def _determine_validation_status(self, res: Dict, baseline_elapsed: Optional[float] = None) -> bool:
        """Determines if the response indicates a successful SSRF (pure owner)."""
        return _pure_determine_validation_status(res, baseline_elapsed=baseline_elapsed)

    def _get_validation_status(self, evidence: Dict) -> str:
        """Tiered validation status — pure owner: ssrf.detection.get_validation_status."""
        return _pure_get_validation_status(evidence)
...
Focus on parameter+target deduplication. Same internal target via different bypasses = DUPLICATE."""
        ...
        try:
            ...
            logger.debug(f"[{self.name}] SSRF dedup JSON parse failed, using fallback: {e}")
            return self._fallback_fingerprint_dedup(wet_findings)

    def _fallback_fingerprint_dedup(self, wet_findings: List[Dict]) -> List[Dict]:
        """Pure owner: ssrf.detection.fallback_fingerprint_dedup."""
        return _pure_fallback_fingerprint_dedup(wet_findings)
    ...
    def _generate_ssrf_fingerprint(self, url: str, parameter: str, payload: str) -> tuple:
        """Pure owner: ssrf.detection.generate_ssrf_fingerprint."""
        return _pure_generate_ssrf_fingerprint(url, parameter, payload)
