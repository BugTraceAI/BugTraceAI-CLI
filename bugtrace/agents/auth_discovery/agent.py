...
class AuthDiscoveryAgent(BaseAgent):
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        for idx, url in enumerate(urls_to_check, 1):
            ...
            if self._v:
                await self._v.aemit("recon.auth.scanning_url", {
                    "url": url,
                    "index": idx,
    ...
    async def _emit_discoveries(self):
        """Emit findings to event bus for orchestrator routing.

        ``EventBus.emit`` is a coroutine and there is no ``publish`` method, so the
        old sync body both fired-and-forgot the emits (coroutine never awaited) and
        raised AttributeError on ``publish``. This is now an async function that
        awaits ``emit`` for both the finding and the auth-token event.
        """
        for jwt_info in self.discovered_jwts:
            ...
            await event_bus.emit("auth_token_found", {
