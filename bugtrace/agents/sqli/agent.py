...
class SQLiAgent(BaseAgent, TechContextMixin):
    ...
    async def _discover_sqli_params(self, url: str, agent_name: str = None) -> Dict[str, str]:
        """Discover params. Delegates to discovery module.

        Accepts the optional `agent_name` positional the dedup pipeline passes when
        this method is used as `discover_fn` (`_discover(url, agent_name)`); we ignore
        it and use `self.name`. Without it, discovery raised "takes 2 positional
        arguments but 3 were given" and SQLi param discovery silently failed.
        """
