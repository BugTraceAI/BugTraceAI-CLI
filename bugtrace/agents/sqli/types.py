...
# Signatures that prove an ACTUAL SQL error was emitted (not merely a page that mentions a
# database). Only these confirm error-based SQLi. The bare DB-name patterns in DB_FINGERPRINTS
# (r"mysql", r"sqlite", r"oracle", r"postgresql", r"pg_", r"mssql", ...) match ANY response that
# mentions a database — a footer, a docs page, a stack-disclosure, the BugStore /api/debug/vulns
# catalog — on ANY target, so they must NEVER confirm on their own; they are only used to LABEL
# which DB after a real error signature is seen. This is target-agnostic (no baseline needed).
SQL_ERROR_SIGNATURES: List[str] = [
    ...
    r"ERROR:\s*syntax error at or near", r"valid PostgreSQL result",
    r"unterminated quoted string at or near",
    # MSSQL
    r"Unclosed quotation mark", r"SqlException", r"Incorrect syntax near",
    r"ODBC SQL Server Driver", r"SQLServer JDBC Driver", r"Procedure.*expects parameter",
    # Oracle
    r"ORA-\d{5}", r"Oracle.*Driver", r"quoted string not properly terminated",
    r"oracle\.jdbc", r"SQL command not properly ended",
    # SQLite
    r"SQLITE_ERROR", r"sqlite3\.OperationalError", r"unrecognized token",
    r"unable to open database file", r"near \".*\": syntax error",
]
