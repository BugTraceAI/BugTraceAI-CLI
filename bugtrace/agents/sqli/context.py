...
def validate_sqli_finding(finding: Dict) -> Tuple[bool, str]:
    ...
    # Quote-parity differential ('->5xx AND balanced ''->baseline) is proof on its own — the
    # status tracking quote balance can only happen when input sits inside a SQL string literal.
    has_quote_parity = bool(evidence.get("quote_parity_confirmed"))

    if not (has_error or has_time or has_data or has_sqlmap or has_oob or has_quote_parity):
        return False, "SQLi requires proof: SQL error, time delay, data extraction, sqlmap confirm, quote-parity, or OOB callback"
