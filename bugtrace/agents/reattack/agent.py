    ...
    async def _reattack_admin_endpoints(self, scan_ctx_id: str, auth_headers: Dict[str, str]):
        ...
        if numeric_id:
            try:
                ...
                if scan_data and "target" in scan_data:
                    target_base = self._base_url(scan_data["target"])
        ...
        if not target_base:
            try:
                ...
                if hasattr(settings, 'LAST_TARGET_URL') and settings.LAST_TARGET_URL:
                    target_base = self._base_url(settings.LAST_TARGET_URL)
                ...
                async with scan_service._lock:
                    for sid, ctx in scan_service._active_scans.items():
                        ...
                        if str(sid) == scan_ctx_id or scan_ctx_id.endswith(str(sid)):
                            target_base = self._base_url(ctx.options.target_url)
        ...
        try:
            async with orchestrator.session(DestinationType.TARGET) as session:
                ...
                # Also probe a small GENERIC wordlist of admin template/preview endpoints commonly
                # prone to SSTI (heuristic — complements the keyword-discovered candidates above;
                # harmless 404 on targets that lack them, not tuned to any one target's routes).
                for path in ("/admin/preview", "/admin/template", "/admin/render",
                             "/admin/email/preview", "/api/admin/preview", "/api/admin/template/render",
                             "/api/admin/email-preview", "/preview", "/render", "/template/render"):
                    url = f"{target_base}{path}"
