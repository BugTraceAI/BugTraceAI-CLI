    ...
    async def drain(self, timeout: Optional[float] = None) -> bool:
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                logger.warning(
                    f"WorkerPool '{self.config.specialist}' drain timed out "
                    f"with {depth} items remaining"
                )
                return False

            if not self._running or not self._workers:
                logger.error(
                    f"WorkerPool '{self.config.specialist}' drain aborted: "
                    f"no live workers and {depth} items remain"
                )
                return False
