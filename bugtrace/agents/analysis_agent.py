...
class DASTySASTAgent(BaseAgent):
    ...
    async def _check_cookie_sqli_probes(self) -> Dict:
        ...
        try:
            ...
            # CRITICAL: use a JAR-LESS session for cookie injection.
            # The pooled session carries aiohttp's default cookie jar, which
            # silently OVERRIDES any manual `Cookie` header with previously
            # stored cookies (e.g. a clean TrackingId set on GET /). That
            # neutralizes every cookie-based injection — the server only ever
            # sees the clean value, so error/time-based SQLi never triggers
            # (root cause of missed cookie SQLi, e.g. TrackingId time-based blind).
            # DummyCookieJar makes aiohttp send our assembled Cookie header verbatim.
            ...
            for _f in findings:
                ...
                if _dedup_key in DASTySASTAgent._emitted_cookie_sqli:
                    continue
                ...
                if _recheck:
                    if not await self._reconfirm_cookie_sqli(_recheck):
                        ...
                        continue
                DASTySASTAgent._emitted_cookie_sqli.add(_dedup_key)
                _ev = _f.get("evidence")
                await event_bus.emit(
                    EventType.VULNERABILITY_DETECTED,
                    {
