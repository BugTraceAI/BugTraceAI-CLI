            ...
            async with session.get(
                ...
                for marker in self.INJECTION_MARKERS:
                    ...
                    for header_name, header_value in resp_headers.items():
                        # Only a marker appearing as a new header NAME proves a split header.
                        # A marker in a header VALUE is mere reflection (e.g. a redirect param
                        # echoed into Location) and was auto-confirming CRLF from reflection.
                        if marker.lower() in header_name.lower():
...
Focus on header name-only deduplication UNLESS parameters are different."""
    ...
    async def start_queue_consumer(self, scan_context: str) -> None:
        ...
        # Count confirmed vulnerabilities — validated results ONLY. Do NOT add the DRY
        # candidate count (it inflated the vuln total with untested/unvalidated endpoints).
    ...
    async def _smart_probe_crlf(self, session: aiohttp.ClientSession, param: str) -> Tuple[bool, Optional[Dict]]:
        ...
        try:
            ...
            async with session.get(
                ...
                # Check 1: Marker as a new header NAME = real split. A marker in a header
                # VALUE is reflection (e.g. the param echoed into Location), not injection.
                for header_name, header_value in resp_headers.items():
                    if "bt-probe" in header_name.lower():
