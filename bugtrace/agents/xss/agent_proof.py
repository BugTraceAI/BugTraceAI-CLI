...
class XSSAgentProofMixin:
    """Finding gate / proof helpers."""

    def _validate_before_emit(self, finding: Dict) -> tuple[bool, str]:
        """XSS emit gate. Pure owner: xss.finding_builder.validate_before_emit."""
        from bugtrace.agents.xss.finding_builder import validate_before_emit

        is_valid, error = super()._validate_before_emit(finding)
        return validate_before_emit(finding, base_is_valid=is_valid, base_error=error)

    def _calculate_confidence(self, reflection: "Reflection") -> float:
        """Pure owner: xss.policy.calculate_confidence."""
        from bugtrace.agents.xss import policy as _pol

        return _pol.calculate_confidence(reflection)
    ...
    def _has_dialog_detected(self, evidence: Dict) -> bool:
        ...
        if _pol.has_dialog_detected(evidence):
    ...
    def _has_vision_proof(self, evidence: Dict, finding_data: Dict = None) -> bool:
        ...
        if _pol.has_vision_proof(evidence):
    ...
    def _has_dom_mutation_proof(self, evidence: Dict) -> bool:
        ...
        if _pol.has_dom_mutation_proof(evidence):
    ...
    def _has_console_execution_proof(self, evidence: Dict) -> bool:
        ...
        if _pol.has_console_execution_proof(evidence):
    ...
    def _has_fragment_xss_with_screenshot(self, finding_data: Dict) -> bool:
        ...
        if _pol.has_fragment_xss_with_screenshot(finding_data):
    ...
    def _should_create_finding(self, test_result: Dict) -> bool:
        """Pure owner: xss.policy.should_create_finding."""
        from bugtrace.agents.xss import policy as _pol

        ok = _pol.should_create_finding(test_result)
        if not ok:
