...
def is_time_based_confirmed(
    ...
    """Return True when the payload adds a delay relative to baseline."""
...
class RCEAgent(BaseAgent, TechContextMixin):
    ...
    def _create_eval_finding(self, param: str, payload: str, target: str, product: str) -> Dict:
        """Create finding for eval-based RCE (evidence carries the real expression/product)."""
        return {
            "type": "RCE",
            "url": self.url,
            "parameter": param,
            ...
            "validated": True,
            "status": "VALIDATED_CONFIRMED",
            "evidence": f"Arithmetic expression '{payload}' evaluated to '{product}' in the response, and '{product}' was absent from a benign baseline (proves evaluation, not reflection).",
            "description": f"Remote Code Execution via eval() confirmed. Parameter '{param}' evaluates arbitrary expressions: '{payload}' returned '{product}'.",
            "reproduction": f"curl '{target}'   # response contains {product}",
            "cwe_id": get_cwe_for_vuln("RCE"),
            "remediation": get_remediation_for_vuln("RCE"),
            "cve_id": "N/A",
            "http_request": f"GET {target}",
            "http_response": f"Computed value {product} present (indicates code evaluation)",
        }
    ...
    async def _test_time_based(self, session, param: str, payload: str) -> Optional[Dict]:
        ...
        baseline = await self._measure_payload(session, "1", param)
        if baseline is None:
            return None

        elapsed = await self._measure_payload(session, payload, param)
        if elapsed is None:
            return None

        if is_time_based_confirmed(baseline, elapsed):
            return self._create_time_based_finding(param, payload, elapsed - baseline)
        return None

    async def _measure_payload(self, session, payload: str, param: str) -> Optional[float]:
        """Inject payload and return elapsed seconds for differential timing."""
        start = time.time()
    ...
    async def _test_eval_based(self, session, param: str, payload: str) -> Optional[Dict]:
        """Test eval-based RCE via an arithmetic differential (no hardcoded output token).

        Inject a distinctive product and confirm ONLY if the computed value appears in the
        response AND is absent from a benign baseline — so a reflected expression or a page
        that merely contains the digits cannot false-positive. Mirrors the CSTI arithmetic
        proof; the incoming `payload` is ignored in favour of this self-verifying probe.
        """
        a = b = 1000003
        expr = f"{a}*{b}"
        product = str(a * b)  # 1000006000009 — distinctive; will not occur by chance
        dashboard.update_task(f"RCE:{param}", status=f"Testing Eval: {expr}")
        probe = self._inject_payload(self.url, param, expr)
        baseline = self._inject_payload(self.url, param, str(a))

        try:
            async with session.get(baseline) as r0:
                base_text = await r0.text()
            async with session.get(probe) as resp:
                text = await resp.text()
                # Evaluated (not reflected / not coincidental): the PRODUCT appears here but
                # not in the baseline. A reflected expression never contains the product
                # substring, so plain reflection cannot satisfy this.
                if product in text and product not in base_text:
                    return self._create_eval_finding(param, expr, probe, product)
    ...
    async def analyze_and_dedup_queue(self) -> List[Dict]:
        ...
        try:
            ...
            logger.debug(f"[{self.name}] LLM dedup failed, using fingerprint fallback: {e}")
...
Treat each parameter as a SEPARATE potential injection point - do not merge them."""
        ...
        try:
            ...
            logger.debug(f"[{self.name}] RCE dedup JSON parse failed, using fallback: {e}")
    ...
    async def _test_output_based(self, session, param: str) -> Optional[Dict]:
        ...
        # Baseline: fetch the URL with a benign value to know what the page normally contains.
        # Without this, a page that contains any lowercase word (e.g. JSON) false-confirms whoami.
        baseline_text = ""
        try:
            baseline_url = self._inject_payload(self.url, param, "btai_baseline_7331")
        ...
        output_payloads = [
            ("id", r"uid=\d+"),
            ("whoami", None),  # handled with differential below
                ...
                async with session.get(target, timeout=10) as resp:
                    ...
                    if resp.status == 200:
                        if cmd == "echo BTAIRCE7331" and "BTAIRCE7331" in text:
                            ...
                            # Differential: find a username-like word in the response
                            # that is NOT in the baseline (new content = command output)
                            match = re.search(r"\b[a-z_][a-z0-9_-]{2,31}\b", text)
                            if match and match.group() not in {param, cmd, "btai_baseline_7331"}:
                                if match.group() not in baseline_text:
                                    return self._create_output_finding(param, cmd, match.group())
            except Exception:
                continue
        return None
