...
def is_time_based_confirmed(
    ...
    """Return True when a response is slow relative to this target's baseline."""
...
class SSRFAgent(BaseAgent, TechContextMixin):
    ...
    def _determine_validation_status(self, res: Dict, baseline_elapsed: Optional[float] = None) -> bool:
        ...
        indicators = [
            ...
            "aws metadata",
            "metadata-flavor",
            "computemetadata/v1"  # NOTE: `text` is lowercased above — literal must be too
        ]
        ...
        # TIMING indicators are only accepted as differential evidence.
        if (
            baseline_elapsed is not None
            and is_time_based_confirmed(baseline_elapsed, res.get("elapsed", 0))
        ):
            return True
...
Focus on parameter+target deduplication. Same internal target via different bypasses = DUPLICATE."""
    ...
    async def start_queue_consumer(self, scan_context: str) -> None:
        ...
        self._v.emit("exploit.specialist.completed", {"agent": "SSRF", "processed": len(dry_list), "vulns": vulns_count, "candidates": candidates_count})
