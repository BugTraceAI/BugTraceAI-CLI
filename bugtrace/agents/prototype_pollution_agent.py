...
class PrototypePollutionAgent(BaseAgent, TechContextMixin):
    ...
    async def _auditor_phase(self, vectors: List[Dict]) -> List[Dict]:
        ...
        for vector in vectors:
            ...
            if result and result.get("exploitable"):
                # Plain marker reflection is not proof of pollution. Only RCE/gadget
                # evidence warrants auto-confirmation; reflection-only detections must
                # be re-validated rather than emitted as VALIDATED_CONFIRMED.
                if not (result.get("rce_confirmed") or result.get("gadget_found")):
    ...
    def _check_rce_output(self, response_text: str) -> Optional[str]:
        ...
        # Only structurally-unambiguous command output confirms RCE. The old whoami word-list
        # (root|admin|node|nobody|ubuntu…) and the loose hostname pattern match benign content
        # ("node" in any Node app, "admin" in nav) — they fabricated CRITICAL RCE and defeated
        # the PENDING_VALIDATION guard downstream. Kept only the two impossible-in-benign ones.
        rce_indicators = [
            (r'uid=\d+.*gid=\d+', "id_output"),   # `id` command output
            (r'root:x:0:0', "passwd_read"),       # /etc/passwd disclosure
        ]
