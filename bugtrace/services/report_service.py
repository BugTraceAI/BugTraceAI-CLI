...
class ReportService:
    ...
    def _build_report_context(self, scan_id: int, target_url: str, scan_config: Dict[str, Any] = None) -> ReportContext:
        ...
        findings = self._load_rich_findings(scan_id)
        if findings is None:
            findings = [
                self._finding_table_to_dict(finding_table)
                for finding_table in self.db.get_findings_for_scan(scan_id)
                if self._is_reportable_status(finding_table.status)
            ]
    ...
    def _load_rich_findings(self, scan_id: int) -> Optional[List[Dict[str, Any]]]:
        """Load report-grade findings from validated_findings.json when available."""
        try:
            from bugtrace.services.scan_service import ScanService

            report_dir = ScanService()._find_report_dir_for_scan(scan_id)
            if not report_dir:
                return None

            path = report_dir / "validated_findings.json"
            if not path.is_file():
                return None

            data = json.loads(path.read_text(encoding="utf-8"))
            findings = self._extract_rich_findings(data)
            reportable = [f for f in findings if self._is_reportable_finding_dict(f)]
            logger.info(
                f"Loaded {len(reportable)} rich findings from {path} for scan {scan_id}"
            )
            return reportable
        except Exception as e:
            logger.warning(f"Failed to load rich findings for scan {scan_id}: {e}")
            return None
    ...
    def _finding_table_to_dict(self, finding: FindingTable) -> Dict[str, Any]:
        ...
        """
        return {
            "type": self._enum_value(finding.type),
            ...
            "validated": (
                finding.visual_validated
                or self._enum_value(finding.status) == "VALIDATED_CONFIRMED"
            ),
