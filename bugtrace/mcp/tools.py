...
def _extract_critical_findings(report_dir: Path, target_url: str) -> str:
    ...
    for i, f in enumerate(critical[:15], 1):
        ...
        # Seed enrichment: show the real authenticated request that confirmed the
        # finding (auth masked at capture time) as a raw HTTP block when available.
        repro_obj = f.get("repro")
        raw_http = f.get("http_request")
        if isinstance(repro_obj, dict):
            cr = repro_obj.get("confirming_request") or {}
            auth = repro_obj.get("auth") or {}
            if cr.get("method") and cr.get("url"):
                lines.append(f"**Confirmed via:** `{cr['method']} {cr['url']}`")
                if auth.get("auth_used"):
                    lines.append(f"**Auth:** {auth.get('auth_scheme', 'header')} (token masked)")
                if raw_http:
                    block = raw_http if len(raw_http) <= 1200 else raw_http[:1200] + "\n...(truncated)"
                    lines.append("```http\n" + block + "\n```")
        lines.append("")
