...
# (Optional) formatting/lint tooling used by the project
...
> **Note:** The automated `pytest` test suite (`tests/`) is **internal-only and is not shipped in the public repository**. The commands in this section apply to the internal development repo; they will not run against a public checkout. If you are contributing from the public repo, validate changes by running a scan against an authorized target instead.
...
- [ ] Tests pass: `pytest tests/ -q` (internal repo only — see the note under [Testing](#testing))
...
- **Architecture**: [README.md](README.md) (Reactor Architecture section)
...
- Verify setup: `python -m bugtrace --help` (the `pytest` sanity suite is internal-only — see [Testing](#testing))
