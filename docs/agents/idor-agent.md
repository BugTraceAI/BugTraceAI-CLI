...
> **Reference example:** This page is the canonical reference example for the per-agent documentation under `docs/agents/`. The other specialist agent docs follow the same structure (overview, pipeline, configuration, phases, safety, examples).
...
- CDP (Chrome DevTools Protocol) is only used for XSS/CSTI/SSTI
